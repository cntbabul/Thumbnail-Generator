...
export const generateSingleThumbnail = async (thumbnailId: string, prompt: string, headshotUrl: string): Promise<void> => {
  try {
...
export const processJob = async (jobId: string): Promise<void> => {
  try {
    ...
    const tasks = thumbnails.map((t) =>
      generateSingleThumbnail(t.id, job.prompt, job.headshot_url)
    );
