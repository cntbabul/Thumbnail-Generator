...
// Routes
app.post('/upload-headshot', upload.single('file'), async (req: Request, res: Response) => {
  try {
    if (!req.file) {
...
app.get('/jobs/:job_id', async (req: Request, res: Response) => {
  try {
    ...
    const thumbnails = (job as any).thumbnails as Thumbnail[] || [];
...
app.get('/jobs/:job_id/stream', async (req: Request, res: Response) => {
